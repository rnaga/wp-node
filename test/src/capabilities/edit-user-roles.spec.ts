...
test("edit_user_roles", async () => {
  await runMulti();
  const superAdminRole = await superAdmin.role();

  expect(superAdminRole.isSuperAdmin()).toBe(true);

  let result = await cap.check("edit_user_roles", superAdmin, [1]);

  expect(result).not.toContain(DO_NOT_ALLOW);

  // Invalid blog id
  result = await cap.check("edit_user_roles", superAdmin, [99]);
  expect(result).toContain(DO_NOT_ALLOW);

  // non admin
  result = await cap.check("edit_user_roles", user, [1]);
  expect(result).toContain(DO_NOT_ALLOW);

  await runSingle();

  // Single site ignores blog id
  result = await cap.check("edit_user_roles", admin, [99]);
  expect(result).not.toContain(DO_NOT_ALLOW);
});
